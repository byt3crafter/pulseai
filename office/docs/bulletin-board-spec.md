...
> First concrete office-system feature for Hermes3D.
...
This should be the first step toward making Hermes3D a real agent operations environment instead of only a gateway visualizer.
...
The feature should hook into systems Hermes3D already has.
...
The board must not depend on provider-specific gateway methods.
...
- the system works with Hermes, custom runtimes, and demo mode
...
The bulletin board should become the first shared memory surface inside Hermes3D.
