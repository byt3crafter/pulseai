...
Scope:
- Describes how Studio connects to the gateway, how runtime streaming arrives over WebSockets, and how the UI renders it.
- Treats **PI** as “the coding agent running behind the Gateway”. Studio does not implement PI logic; it displays and controls the Gateway session.

Non-scope:
- PI internals and model/tool execution details. Those live in the backend behind the gateway.
...
- Browser gateway protocol client: `src/lib/gateway/protocol/GatewayBrowserClient.ts`
...
Studio owns the browser client used to speak the Gateway protocol:
- Client: `src/lib/gateway/protocol/GatewayBrowserClient.ts`
- Event normalization: `src/lib/runtime/normalizeGatewayEvent.ts`

Important:
- That client implements Hermes3D gateway protocol v3.
- If a protocol mismatch is suspected (missing event fields, renamed streams, different error codes), check the protocol version reported in the `connect` response against what the client expects.

## Behavior Source Of Truth

For chat streaming behavior, the authoritative surfaces are:
- The gateway `connect` handshake and its advertised `minProtocol`/`maxProtocol`.
- `chat.send` ack + idempotency behavior, and `chat.history` payload shaping/sanitization.
- `agent` event fanout and the synthetic `chat` delta/final bridging the gateway performs.
- `assistant`/`tool`/`lifecycle` stream emission during a run.

The bundled adapters in `server/hermes-gateway-adapter.js` and
`server/demo-gateway-adapter.js` are the two implementations of that contract
this repo controls, so they are the fastest place to verify behavior.

## Terminology

- Studio: this repo, a Next.js UI with a custom Node server.
- Gateway (upstream): the gateway WebSocket server Studio proxies to (default `ws://localhost:18789`).
...
sequenceDiagram
  ...
  participant G as Gateway (upstream)
...
- `~/.hermes/hermes3d/settings.json` (see `README.md` for the canonical location)
...
- The Studio server resolves an upstream token from `hermes3d/settings.json`, and if it is missing it may fall back to the local Hermes config in `hermes.json` (token + port). This behavior exists in both the WS proxy path (`server/studio-settings.js`) and the `/api/studio` storage layer (`src/lib/studio/settings-store.ts`) and they should remain consistent.
...
- The browser-side Gateway client (`src/lib/gateway/protocol/GatewayBrowserClient.ts`) closes the WebSocket with close code `4008` and a reason like `connect failed: <CODE> <MESSAGE>` after it receives a failed `res(connect)`. `GatewayClient.connect()` parses that close into `GatewayResponseError(code, message)` for UI retry policy and user-facing errors.
...
- Transport reconnect (after a successful hello): the browser client reconnects the browser->Studio WebSocket with backoff when it closes, and continues emitting events after reconnect. See `src/lib/gateway/protocol/GatewayBrowserClient.ts`.
- Initial connect failure retry: when the initial `connect` handshake fails (for example bad token), `GatewayClient.connect()` tears down the protocol client and returns a rejected promise; `useGatewayConnection()` may schedule a limited re-attempt unless the error code is known non-retryable. See `resolveGatewayAutoRetryDelayMs` in `src/lib/gateway/GatewayClient.ts`.
...
Gateway event frames may include `seq`. The browser client tracks `seq` and reports gaps (`expected`, `received`) via `onGap`.
...
Files:
- Gap detection: `src/lib/gateway/protocol/GatewayBrowserClient.ts`
