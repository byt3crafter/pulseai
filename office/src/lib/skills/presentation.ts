...
const WORKSPACE_SOURCES = new Set(["hermes-workspace", "agents-skills-personal", "agents-skills-project"]);
...
const resolveGroupId = (skill: SkillStatusEntry): SkillSourceGroupId => {
  ...
  const bundled = skill.bundled || source === "hermes-bundled";
  if (bundled) return "built-in";
  if (WORKSPACE_SOURCES.has(source)) return "workspace";
  if (source === "hermes-managed") return "installed";
  if (source === "hermes-extra") return "extra";
  return "other";
};
...
export const buildSkillMissingDetails = (skill: SkillStatusEntry): string[] => {
  ...
  if (config.length > 0) {
    details.push(`Missing config values (set in hermes.json): ${config.join(", ")}`);
  }
...
export const isBundledBlockedSkill = (skill: SkillStatusEntry): boolean => {
  const source = trimNonEmpty(skill.source) ?? "";
  return (skill.bundled || source === "hermes-bundled") && !skill.eligible;
};
