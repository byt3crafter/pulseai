...
export default function NewAgentClient({ connectedProviders }: { connectedProviders: string[] }) {
    ...
    return (
        <div className="p-4 sm:p-6 lg:p-8 max-w-3xl mx-auto">
            ...
            <h1 className="text-2xl font-bold text-pulse-text">Create an agent</h1>
            <p className="text-sm text-pulse-muted mt-1">Describe what you need and we&apos;ll draft it — or fill in the details yourself. You can refine everything later.</p>
            ...
            <form onSubmit={handleSubmit} className="mt-8 space-y-8">
                ...
                {/* Step 1 — Describe */}
                <section className="rounded-xl border border-pulse-border-subtle bg-pulse-tint p-6">
                    <div className="flex items-center gap-2">
                        ...
                        <h2 className="text-base font-semibold text-pulse-text">Describe your agent — we&apos;ll build it</h2>
                    </div>
                    <p className="text-sm text-pulse-muted mt-1 ml-8">Tell us what it should do. We&apos;ll draft a name, pick a model, and write the starting instructions.</p>
                    ...
                    <textarea
                        ...
                        className="mt-4 w-full px-3.5 py-2.5 border border-pulse-border rounded-lg text-sm focus:ring-2 focus:ring-indigo-500 outline-none transition-shadow bg-pulse-panel text-pulse-text placeholder:text-pulse-faint resize-y disabled:bg-pulse-panel-alt disabled:text-pulse-faint"
                    />
                    <div className="flex flex-wrap gap-1.5 mt-3">
                        {EXAMPLE_DESCRIPTIONS.map((ex) => (
                            <button key={ex} type="button" onClick={() => setDescription(ex)} title={ex}
                                className="text-xs px-2.5 py-1 rounded-full border border-indigo-500/30 bg-pulse-panel text-indigo-500 hover:bg-indigo-500/10 transition-colors motion-reduce:transition-none cursor-pointer max-w-[240px] truncate focus-visible:ring-2 focus-visible:ring-indigo-500 outline-none">
                                {ex.split(".")[0]}
                            </button>
                        ))}
                    </div>
                ...
                <section ref={reviewRef} className="space-y-5 scroll-mt-4">
                    <div className="flex items-center gap-2">
                        ...
                        <h2 className="text-base font-semibold text-pulse-text">Review &amp; edit</h2>
                    </div>
                    ...
                    <div>
                        ...
                        <textarea id="agent-soul" name="systemPrompt" rows={8} value={soul} onChange={(e) => setSoul(e.target.value)}
                            ...
                            className="w-full px-3.5 py-2.5 border border-pulse-border rounded-lg text-sm focus:ring-2 focus:ring-indigo-500 outline-none transition-shadow bg-pulse-panel text-pulse-text placeholder:text-pulse-faint resize-y" />
                    </div>
                </section>
                ...
                <section className="space-y-3">
                    ...
                    <div className="flex items-start bg-red-500/10 p-4 rounded-lg border border-red-500/30">
                        <div className="flex items-center h-5">
                            <input id="agent-sandbox" name="dockerSandboxEnabled" type="checkbox" value="true" className="h-4 w-4 rounded border-pulse-border text-indigo-600 focus:ring-indigo-600 focus:ring-offset-2 focus:ring-offset-pulse-panel cursor-pointer" />
                        </div>
