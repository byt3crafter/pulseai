...
export default function ToolEditor({ agents, tool }: { agents: Agent[]; tool?: ToolData }) {
    ...
    return (
        <div className="p-8 max-w-3xl mx-auto">
            <Link href="/dashboard/tools" className="inline-flex items-center gap-1.5 text-sm text-slate-500 hover:text-slate-800 transition-colors motion-reduce:transition-none mb-4">
                <ArrowLeftIcon className="w-4 h-4" aria-hidden="true" /> Custom Tools
            </Link>
            <h1 className="text-2xl font-bold text-slate-900">{isEdit ? "Edit tool" : "New tool"}</h1>
            ...
            <form onSubmit={handleSubmit} className="mt-8 space-y-8">
                ...
                <section className="space-y-5">
                    ...
                    <div className="grid sm:grid-cols-[2fr_1fr] gap-4">
                        <div>
                            <div className="flex items-center gap-1.5 mb-1.5">
                                <label htmlFor="tool-name" className="block text-sm font-medium text-slate-700">Tool name <span className="text-red-500">*</span></label>
                            ...
                            <input id="tool-name" name="name" type="text" required defaultValue={tool?.name} placeholder="get_order_status"
                                className="w-full px-3.5 py-2.5 border border-slate-300 rounded-lg text-sm font-mono focus:ring-2 focus:ring-indigo-500 outline-none transition-shadow text-slate-900" />
                        </div>
                        <div>
                            <div className="flex items-center gap-1.5 mb-1.5">
                                <label htmlFor="tool-method" className="block text-sm font-medium text-slate-700">Method</label>
                    ...
                    <div>
                        <div className="flex items-center gap-1.5 mb-1.5">
                            <label htmlFor="tool-description" className="block text-sm font-medium text-slate-700">Description <span className="text-red-500">*</span></label>
                    ...
                    <div>
                        <div className="flex items-center gap-1.5 mb-1.5">
                            <label htmlFor="tool-url" className="block text-sm font-medium text-slate-700">URL <span className="text-red-500">*</span></label>
                ...
                <section className="space-y-3">
                    ...
                    <p className="text-sm text-slate-500">What the AI provides when it calls this tool.</p>

                    <div className="space-y-3">
                        {params.map((p, i) => (
                            <div key={i} className="grid grid-cols-1 sm:grid-cols-[1fr_120px_1fr_auto_auto] gap-2 sm:items-center rounded-lg border border-slate-200 p-3 bg-slate-50/50">
                                <input aria-label="Parameter name" value={p.name} onChange={(e) => setParams(params.map((x, j) => j === i ? { ...x, name: e.target.value } : x))} placeholder="orderId"
                                    className="px-3 py-2 border border-slate-300 rounded-lg text-sm font-mono focus:ring-2 focus:ring-indigo-500 outline-none transition-shadow text-slate-900 bg-white" />
                                <select aria-label="Parameter type" value={p.type} onChange={(e) => setParams(params.map((x, j) => j === i ? { ...x, type: e.target.value } : x))}
                                    className="px-3 py-2 border border-slate-300 rounded-lg text-sm focus:ring-2 focus:ring-indigo-500 outline-none transition-shadow text-slate-900 bg-white">
                                ...
                                <input aria-label="Parameter description" value={p.description} onChange={(e) => setParams(params.map((x, j) => j === i ? { ...x, description: e.target.value } : x))} placeholder="Description"
                                    className="px-3 py-2 border border-slate-300 rounded-lg text-sm focus:ring-2 focus:ring-indigo-500 outline-none transition-shadow text-slate-900 bg-white" />
                                <label className="flex items-center gap-1.5 text-xs text-slate-600 whitespace-nowrap px-1">
                                    <input type="checkbox" checked={p.required} onChange={(e) => setParams(params.map((x, j) => j === i ? { ...x, required: e.target.checked } : x))}
                                        className="h-4 w-4 rounded border-slate-300 text-indigo-600 focus:ring-indigo-600 focus:ring-offset-2 cursor-pointer" />
                                    Required
                                </label>
                                <button type="button" onClick={() => setParams(params.filter((_, j) => j !== i))} aria-label="Remove parameter"
                                    className="p-1.5 rounded-md text-slate-400 hover:text-red-600 hover:bg-red-50 transition-colors motion-reduce:transition-none cursor-pointer outline-none focus-visible:ring-2 focus-visible:ring-indigo-500 justify-self-end">
                                    <TrashIcon className="w-4 h-4" aria-hidden="true" />
                                </button>
                            </div>
                        ))}
                        {params.length === 0 && <p className="text-sm text-slate-400 italic">No parameters — the tool is called with no input.</p>}
                    </div>
                    <button type="button" onClick={() => setParams([...params, { name: "", type: "string", description: "", required: false }])}
                        className="inline-flex items-center gap-1.5 text-sm text-indigo-600 hover:text-indigo-700 font-medium transition-colors motion-reduce:transition-none cursor-pointer outline-none focus-visible:ring-2 focus-visible:ring-indigo-500 rounded-md">
                ...
                <section className="space-y-3">
                    <div className="flex items-center gap-1.5">
                        <h2 className="text-base font-semibold text-slate-900">Headers &amp; authentication</h2>
                        <Tooltip text="Sent with every request — for API keys or bearer tokens. Values are encrypted at rest." />
                    </div>
                    <p className="text-sm text-slate-500">Values are stored encrypted and never shown in plaintext elsewhere.</p>

                    <div className="space-y-3">
                        {headers.map((h, i) => (
                            <div key={i} className="grid grid-cols-1 sm:grid-cols-[1fr_1fr_auto] gap-2 sm:items-center rounded-lg border border-slate-200 p-3 bg-slate-50/50">
                        ...
                        {headers.length === 0 && <p className="text-sm text-slate-400 italic">No headers configured.</p>}
                    </div>
                ...
                <section className="space-y-3">
                    <div className="flex items-center gap-1.5">
                        <h2 className="text-base font-semibold text-slate-900">Agent access</h2>
                    ...
                    {agents.length === 0 ? (
                        <p className="text-sm text-slate-400 italic">No agents yet — this tool will be available to any agent you create.</p>
                    ) : (
                ...
                {/* Advanced */}
                <details className="group rounded-lg border border-slate-200 open:bg-slate-50/50">
                    <summary className="cursor-pointer select-none list-none flex items-center justify-between p-4 text-sm font-medium text-slate-700 outline-none focus-visible:ring-2 focus-visible:ring-indigo-500 rounded-lg">
                        <span>Advanced — request body &amp; timeout</span>
                        <ChevronDownIcon className="w-4 h-4 text-slate-400 transition-transform motion-reduce:transition-none group-open:rotate-180" aria-hidden="true" />
                    </summary>
                    <div className="px-4 pb-4 space-y-4">
                        <div>
                            <div className="flex items-center gap-1.5 mb-1.5">
                                <label htmlFor="tool-body" className="block text-sm font-medium text-slate-700">Body template</label>
                            ...
                            <textarea id="tool-body" name="bodyTemplate" rows={3} defaultValue={tool?.bodyTemplate} placeholder='{"id": "{orderId}"}'
                                className="w-full px-3.5 py-2.5 border border-slate-300 rounded-lg text-sm font-mono focus:ring-2 focus:ring-indigo-500 outline-none transition-shadow text-slate-900 resize-y bg-white" />
                        </div>
                        <div>
                            <div className="flex items-center gap-1.5 mb-1.5">
                                <label htmlFor="tool-timeout" className="block text-sm font-medium text-slate-700">Timeout (ms)</label>
                            ...
                            <input id="tool-timeout" name="timeoutMs" type="number" min={1000} max={30000} defaultValue={tool?.timeoutMs || 15000}
                                className="w-40 px-3.5 py-2.5 border border-slate-300 rounded-lg text-sm focus:ring-2 focus:ring-indigo-500 outline-none transition-shadow text-slate-900 bg-white" />
                        </div>
                    </div>
                </details>
                ...
                <div className="flex gap-3 justify-end border-t border-slate-100 pt-5">
                    <Link href="/dashboard/tools" className="px-4 py-2 text-sm font-medium text-slate-700 bg-white border border-slate-300 rounded-lg hover:bg-slate-50 transition-colors motion-reduce:transition-none">Cancel</Link>
